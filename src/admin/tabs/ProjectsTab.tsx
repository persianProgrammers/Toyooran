...
export const ProjectsTab: React.FC = () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const fullProj: Project = {
      ...
      image: formData.image || 'https://images.unsplash.com/photo-1548550023-2bdb3c5beed7?auto=format&fit=crop&w=800&q=80',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredProjects.map((p) => (
          <div key={p.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm flex flex-col hover:border-slate-700 transition-all">
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-6 sm:p-8 max-w-2xl w-full my-8 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">آدرس تصویر پروژه (URL)</label>
                <input
                  type="text"
