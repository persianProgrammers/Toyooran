            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-2xl text-sm font-bold transition-all duration-300  border ${
                activeFilter === tab.id
                  ...
                  : 'bg-white/95 text-slate-600 border-white/40 hover:bg-white hover:text-[#003F86] hover:border-blue-200 shadow-sm'
              }`}
            >
              {tab.label}
            </button>
        ...
        <motion.div  className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {displayedProjects.map((project, index) => (
              <motion.div
                ...
                key={project.id}
                className="relative bg-white/95  transform-gpu will-change-transform rounded-[2rem] border border-white/80 overflow-hidden shadow-lg shadow-slate-200/50 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 flex flex-col group cursor-pointer origin-center"
              ...
              >
                ...
                <div className="relative h-56 sm:h-64 overflow-hidden bg-slate-200">
                  ...
                  {/* Type Badge */}
                  <div className="absolute top-4 right-4 z-20 bg-white/20  border border-white/30 text-white font-bold text-xs px-3 py-1.5 rounded-xl shadow-lg">
                    {project.typeTitle}
                  </div>

                  {/* Location & Year */}
                  <div className="absolute bottom-4 right-4 left-4 z-20 flex items-center justify-between text-white text-xs font-medium">
                    <span className="flex items-center gap-1.5 bg-slate-900/95  border border-white/20 px-2.5 py-1 rounded-lg">
